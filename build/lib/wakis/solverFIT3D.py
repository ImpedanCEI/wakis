...
class SolverFIT3D(PlotMixin, RoutinesMixin):
    def __init__(
        ...
        self.use_conductivity = (
            False  # Will turn true if conductive material or pml is added
        )
        ...
        if self.use_mpi:
            if self.grid.use_mpi:
                ...
                print(
                    "[!] Grid not subdivided for MPI, set `use_mpi`=True also in `GridFIT3D` to enable MPI"
                )
        ...
        self.imu = Field(self.Nx, self.Ny, self.Nz, use_ones=True, dtype=self.dtype) * (
            1.0 / self.mu_bg
        )
        self.sigma = (
        ...
        if dt is None:
            self.dt = cfln / (
                c_light
                * np.sqrt(
                    1 / self.grid.dx**2 + 1 / self.grid.dy**2 + 1 / self.grid.dz**2
                )
            )
        else:
            self.dt = dt
