...
from conductors import OutRect, Plane, ConductorsAssembly, OutCircle
...
for ii in range(Nx):
    for jj in range(Ny):
        ...
        if grid.flag_int_cell[ii, jj]:
            solver.Hz[ii, jj] = (
                jv(1, r * k) * np.cos(theta) * np.cos(0)
            )  # -k*c_light*solver.dt/2)
...
for t in tqdm(range(Nt)):
    ...
    im1 = axs[0].imshow(
        solver.Ex, cmap="jet", vmax=80, vmin=-80
    )  # ,extent=[0, L , 0, L ])
