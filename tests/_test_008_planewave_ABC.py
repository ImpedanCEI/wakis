...
# solver setup
global solver
solver = SolverFIT3D(grid, use_stl=False, 
                     bc_low=bc_low, 
                     bc_high=bc_high,
                     bg='vacuum',
                     dt=0.5*grid.dz/c)
...
#                    f=f, beta=1.0)
source = WavePacket(xs=slice(0, Nx), ys=slice(0,Ny), zs=0,
                    sigmaz=(grid.zmax-grid.zmin)/6, 
                    f=f, tinj=(grid.zmax-grid.zmin))

# plotting
kwargs = {'field':'H', 'component':'y',
                'plane':'ZY', 'pos':0.5, 
                'title':'tests/008_img/Hy', 'vmin':-2, 'vmax':2,
                'cmap':'rainbow', 'patch_reverse':False, 
                'off_screen': True, 'interpolation':'spline36'}

def callback(solver, t):
    n = int(t/solver.dt)
    if n % 20 == 0:
        solver.plot1D(field='H', component='y', line='z', pos=[0.6, 0.5, 0.4], 
               xscale='linear', yscale='linear', xlim=None, #ylim=(-2, 2), 
               figsize=[8,4], title='tests/008_img/Hy1d', off_screen=True, n=n)
        
        # fig, ax = solver.H.inspect(show=False, handles=True)
        # fig.savefig('tests/008_img/Hy_inspect'+str(n).zfill(5))
        # plt.close(fig)

Nt = int(2.0*(solver.z.max()-solver.z.min())/c/solver.dt+(grid.zmax-grid.zmin)/c/solver.dt)
solver.emsolve(Nt, source, callback,
               plot=True,
               plot_every=20,
               **kwargs)
