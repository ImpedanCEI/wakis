...
def analytic_sol_Hz(x, y, z, t):
    ...
    return (
        np.cos(m * np.pi / Lx * (x_0 - Lx / 2))
        * np.cos(n * np.pi / Ly * (y_0 - Ly / 2))
        * np.sin(p * np.pi / Lz * (z_0 - Lz / 2))
...
def analytic_sol_Hy(x, y, z, t):
    ...
    return (
        ...
        * (p * np.pi / Lz)
        * np.cos(m * np.pi / Lx * (x_0 - Lx / 2))
        * np.sin(n * np.pi / Ly * (y_0 - Ly / 2))
        * np.cos(p * np.pi / Lz * (z_0 - Lz / 2))
        * np.cos(np.sqrt(2) * np.pi / Lx * c_light * t)
    )
...
solver = SolverFIT3D(
    grid, bc_low=bc_low, bc_high=bc_high, use_stl=True, bg=[np.inf, 1.0]
)  # PEC backgroung
...
def get_slice(plane):
    ...
    return xx, yy, zz, title, xax, yax, lims
        ...
        for jj in range(Ny):
            for kk in range(Nz):
                if mask[ii, jj, kk]:
                    ...
                    y = (jj + 0.5) * dy + ymin
                    z = (kk + 0.5) * dz + zmin
                    analytic.H[ii, jj, kk, "z"] = analytic_sol_Hz(
                        x, y, z, (n - 0.5) * solver.dt
                    )
                    analytic.H[ii, jj, kk, "y"] = analytic_sol_Hy(
                        x, y, z, (n - 0.5) * solver.dt
                    )
                    analytic.H[ii, jj, kk, "x"] = analytic_sol_Hx(
                        x, y, z, (n - 0.5) * solver.dt
                    )
...
def plot_E_field(solver, n):
    ...
    # FIT
    for j, plane in enumerate(["XY", "ZY", "ZX"]):
        for i, ax in enumerate(axs[j, :]):
            xx, yy, zz, title, xax, yax, _ = get_slice(plane)
            vmin, vmax = -lims[i], lims[i]
            im = ax.imshow(
                solver.E[xx, yy, zz, dims[i]], cmap="rainbow", vmin=vmin, vmax=vmax
            )
            fig.colorbar(
                im,
...
def plot_H_field(solver, n):
    ...
    for j, plane in enumerate(["XY", "ZY", "ZX"]):
        for i, ax in enumerate(axs[j, :]):
            xx, yy, zz, title, xax, yax, lims = get_slice(plane)
            vmin, vmax = -lims[i], lims[i]
            im = ax.imshow(
                solver.H[xx, yy, zz, dims[i]], cmap="rainbow", vmin=vmin, vmax=vmax
            )
            fig.colorbar(
                im,
                cax=make_axes_locatable(ax).append_axes("right", size="5%", pad=0.05),
...
def plot_analytic(solver, n):
    fig, axs = plt.subplots(3, 3, tight_layout=True, figsize=[8, 6])
    dims = {0: "x", 1: "y", 2: "z"}

    # FIT
    get_analytic_H(solver, n)
    for j, plane in enumerate(["XY", "ZY", "ZX"]):
        for i, ax in enumerate(axs[j, :]):
            xx, yy, zz, title, xax, yax, lims = get_slice(plane)
            vmin, vmax = -lims[i], lims[i]
    ...
    fig.suptitle(f"Analytic H, timestep={n}")
    fig.savefig(f"{folder}H/" + str(n).zfill(4) + ".png")
    plt.clf()
    plt.close(fig)
...
# No zero waves for E IC, zero waves visible in ZY/ZX on Hx field (v5), (v6 + analytic=1)
# mask_x = np.roll(mask, [-1], axis=[1])+np.roll(mask, [+1], axis=[1])
# mask_y = np.roll(mask, [-1], axis=[0])+np.roll(mask, [+1], axis=[0])
# mask_z = mask_x+mask_y

# v8 - only a corner was missing, issues in Hy
# mask_x = mask + np.roll(mask, [-1], axis=[1]) + np.roll(mask, [-1], axis=[2])
# mask_y = mask
# mask_z = mask + np.roll(mask, [-1], axis=[0]) + np.roll(mask, [-1], axis=[1])

# v9 issues with corner hi-hi, issues in Hy
# mask_x = np.roll(mask + np.roll(mask, [-1], axis=[1]),  [-1], axis=[2]) + mask
# mask_y = mask
# mask_z = np.roll(mask + np.roll(mask, [-1], axis=[0]),  [-1], axis=[1]) + mask

# v10 - best so far, no corners, only some waves in E starting from low y in Ex and Ez
# mask_x = mask + np.roll(mask, [-1], axis=[1]) + np.roll(mask, [-1], axis=[2])
# mask_y = mask + np.roll(mask, [-1], axis=[0]) + np.roll(mask, [-1], axis=[2])
# mask_z = mask + np.roll(mask, [-1], axis=[0]) + np.roll(mask, [-1], axis=[1])

# v11 - issues with corners hi hi, same waves, coming from HzXY and HX YZ
# mask_x = mask + np.roll(mask + np.roll(mask, [-1], axis=[1]),  [-1], axis=[2])
# mask_y = mask + np.roll(mask + np.roll(mask, [-1], axis=[0]),  [-1], axis=[2])
# mask_z = mask + np.roll(mask + np.roll(mask, [-1], axis=[0]),  [-1], axis=[1])

# v12
mask_x = mask + np.roll(mask, [-1], axis=[1])
mask_x += np.roll(mask_x, [-1], axis=[2])
mask_y = mask + np.roll(mask, [-1], axis=[0])
mask_y += np.roll(mask_y, [-1], axis=[2])
mask_z = mask + np.roll(mask, [-1], axis=[0])
mask_z += np.roll(mask_z, [-1], axis=[1])
...
for nt in tqdm(range(Nt)):
    ...
    if nt % 2:
        ...
        plot_H_field(solver, nt)
        # plot_analytic(analytic, nt)
