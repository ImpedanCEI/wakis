...
if run:
    ...
    if plot:
        ...
        ax.set_ylim(-100, 100)
        ax.set_zticks(
            np.linspace(0, np.max(field), 2),
            labels=["0", "{:.2e}".format(np.max(field))],
        )
...
R = np.abs(ht["Ez"][0 + n_pml : 76 + n_pml, :]) - np.abs(
    hr["Ez"][75:151, :]
)  # [76x201]
...
if plot:
    ...
    ax.get_proj = lambda: np.dot(Axes3D.get_proj(ax), np.diag([1.0, 1.0, 0.4, 1.0]))
