...
class GridFIT3D:
    ...
    def _mpi_initialize(self):
        ...
        if self.Nz < self.size:
            raise ValueError(
                f"Nz ({self.Nz}) must be greater than or equal to the number of MPI processes ({self.size})."
            )
        ...
        if type(self.stl_colors) is not dict:
            if self.stl_colors is None:
                ...
                try:
                    ...
                    self._assign_colors()
                    print(
                        "[!] If `stl_colors` is a list, it must have the same length as `stl_solids`."
                    )
    ...
    def _refine_xyz_axis(self, method="insert", tol=1e-6):
        ...
        if self.verbose:
            print(
                f"Refined grid: Nx = {len(self.x)}, Ny ={len(self.y)}, Nz = {len(self.z)}"
            )
