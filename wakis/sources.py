...
class Pulse:
    ...
    def harris_pulse(self, t):
        ...
        def _pulse(x):
            return (
                10
                - 15 * np.cos(2 * np.pi / self.L * x)
                + 6 * np.cos(4 * np.pi / self.L * x)
                - np.cos(6 * np.pi / self.L * x)
            ) / 32

        # handle scalar or array t
        t = np.asarray(t)  # ensures array operations work

        # use np.where to apply the condition elementwise
        return np.where(t < self.L, _pulse(t), 0.0)
